    ...
    def forward(self,
        ...
        if attn_bias is None:
            if input_mask is None:
                input_mask = fluid.layers.cast(src_ids != 0, 'float32')
            ...
            if use_causal_mask:
                sequence = fluid.layers.reshape(fluid.layers.range(0, d_seqlen, 1, dtype='float32') + 1., [1, 1, -1, 1])
                causal_mask = fluid.layers.cast((fluid.layers.matmul(sequence, 1. / sequence, transpose_y=True) >= 1.),
                                                'float32')
...
if __name__ == "__main__":
    ...
    with fluid.dygraph.guard(place):
        ernie = ErnieModel()
        data = dygraph.to_variable(np_data)
        pooled, encoded = ernie(data)
        print(pooled.shape)
