...
from paddleformers.transformers.video_utils import VideoMetadata
...
def prepare_video_inputs(
    ...
    one specifies return_tensors="np", or a list of list of Paddle tensors if one specifies return_tensors="pd".
...
class VideoProcessingTestMixin:
    ...
    def test_cast_dtype_device(self):
        for video_processing_class in self.video_processor_list:
            if self.test_cast_dtype is not None:
                ...
                encoding = video_processor(video_inputs, return_tensors="pd")

                self.assertNotEqual(encoding[self.input_name].place, paddle.CPUPlace())
                self.assertEqual(encoding[self.input_name].dtype, paddle.float32)

                encoding = video_processor(video_inputs, return_tensors="pd").to(paddle.float16)
                self.assertNotEqual(encoding[self.input_name].place, paddle.CPUPlace())
                self.assertEqual(encoding[self.input_name].dtype, paddle.float16)

                encoding = video_processor(video_inputs, return_tensors="pd").to(paddle.bfloat16)
                self.assertNotEqual(encoding[self.input_name].place, paddle.CPUPlace())
                self.assertEqual(encoding[self.input_name].dtype, paddle.bfloat16)

                # Test text + video feature case
                encoding = video_processor(video_inputs, return_tensors="pd")
                encoding.update({"input_ids": paddle.Tensor([[1, 2, 3], [4, 5, 6]])})
                encoding = encoding.to(paddle.float16)
    ...
    def test_call_sample_frames(self):
        for video_processing_class in self.video_processor_list:
            ...
            encoded_videos = video_processing(video_inputs[0], return_tensors="pd", num_frames=3)[self.input_name]
            # encoded_videos_batched = video_processing(video_inputs, return_tensors="pd", num_frames=3)[self.input_name]
            ...
            #     video_inputs, return_tensors="pd", fps=3, video_metadata=batched_metadata
            # )[self.input_name]
            ...
            with self.assertRaises(ValueError):
                encoded_videos = video_processing(video_inputs[0], return_tensors="pd", num_frames=10)[self.input_name]
                # encoded_videos_batched = video_processing(video_inputs, return_tensors="pd", num_frames=10)[
                #     self.input_name
                # ]
